...
> ℹ️ **Additive, not breaking (`payloadFailureAttachesResolvedItem` capability).** The standard write-actions battery's `WriteTestCapabilities` gains `payloadFailureAttachesResolvedItem` (default `true`). An implementation that refuses a mistyped update value *before* it reads the addressed row has no merged row to attach to the failure, so it declares `false`; the battery then registers the §11.4 leaf that demands the resolved post-merge item as a visible skip, and still holds the implementation to the offending row's `item_pk` and to any item it does attach (which must be the merged row). An implementation that reads the row must attach it and leaves the flag alone. Nothing changes for an implementation that omits the flag.

> ⚠️ **Breaking (`custom` failures are unrecoverable).** A per-action failure whose error is `custom` — a verb that cannot apply to the value it found or was given: `inc` on a `null`, non-number or `NaN` field; `push`, `pull` or `add_to_set` on a `null` or non-array field; `add_to_set` with `unique_by: 'pk'` whose items are not objects carrying the list's key, or whose list path the DDL declares no rules for — carries `unrecoverable: true`, as every other error class does. The one class that leaves the flag unset is `blocked`, which records that the action never ran. The flag is a verdict on the action against the rows the engine was given: re-sending the same action against the same rows draws the same refusal. A layer that re-sends failed writes on the strength of the flag (an outbox, a sync loop, a retry wrapper) used to treat these refusals as transient; it now sees them as final and surfaces the refusal to its caller, which decides whether to submit a corrected write. A layer that runs the engine over a *copy* of the rows before sending (an optimistic pre-check) learns only how the write fares against that copy. Every `custom` refusal that reads a row's current value (all of the above except the `unique_by: 'pk'` payload and DDL faults) may be answered differently by the store that holds the rows, so such a layer forwards the write for that store to adjudicate rather than refusing it locally — the treatment a `create_duplicated_key` from a pre-check already calls for, since the store may not hold that key. Forwarding a payload or DDL fault too is harmless: the store refuses it with the same flag, and the retry stops there. The standard write-actions battery pins the flag on the engine's own refusals (§11.2 T-11.6 and T-11.12, §12.6–12.8 including a nested `array_scope` leaf), so an implementation that builds its own failure envelope rather than relaying the engine's must flag those refusals to stay conformant. Refusals of an implementation's own kind (a lock, a quota, a policy) are its own to classify, per outcome, as before. Nothing else changes: the same actions are refused, with the same `message`, and the row is left as it was.
