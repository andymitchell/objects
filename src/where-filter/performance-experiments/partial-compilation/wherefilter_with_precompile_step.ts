...
type PartialObjectFilter2<
  T extends Record<string, any>,
  TPaths extends DotPropPathsIncArrayUnion<T> = DotPropPathsIncArrayUnion<T> 
> = Partial<{
  [P in TPaths]: IsAssignableTo<P, DotPropPathToArraySpreadingArrays<T>> extends true
    ? ArrayFilter<PathValueIncDiscrimatedUnions<T, P>>
    : ValueComparison<PathValueIncDiscrimatedUnions<T, P>>;
}>;

// Optimized WhereFilterDefinition
type WhereFilterDefinition2<
  T extends Record<string, any>,
  TPaths extends DotPropPathsIncArrayUnion<T>  = DotPropPathsIncArrayUnion<T>
> = PartialObjectFilter2<T, TPaths> | LogicFilter<T>;

export type UserPaths = DotPropPathsIncArrayUnion<User>;
type UserFilter = WhereFilterDefinition2<User, UserPaths>;

    
/**
 * I imagine this would be used like full-compilation, where UserPaths is fully computed/expanded/flattened with ts-morph, and used to 
 * generate a lighter UserFilter = WhereFilterDefinition2
 * 
 * The trouble is I don't know if it's faster. https://github.com/microsoft/typescript-analyze-trace probably helps, but its needs a decent simulation of complex types and the different approaches. 
 */