...
4. At each **leaf**, delegate to a dialect-specific `IPropertyTranslator` which:
   - Resolves the dot-prop path to a SQL accessor (e.g. `data->'contact'->>'name'` for Postgres, `json_extract(data, '$.contact.name')` for SQLite)
   - Converts the value comparison (`$gt`, `$in`, `$contains`, scalar, etc.) into a SQL fragment with parameterised placeholders
