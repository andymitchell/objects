...
import type { Tags, BoolFlags, BoolLeaf, ObjArray } from "./fixtures.ts";
...
export function registerArraySemantics(ctx: SectionCtx): void {
    ...
    describe('18. Array semantics (deep)', () => {
        ...
        /**
         * `$elemMatch` asks a question only an array can answer: the value AT the path must ITSELF be an array,
         * and one of its own elements must satisfy the body. MongoDB answers `false` for a scalar there, and §18.9
         * already holds every engine to that on a plain scalar field. This block holds them to the same rule one
         * level down, where a path descends THROUGH an array to a leaf.
         *
         * The distinction is not cosmetic. A path like `items.k` yields one leaf per element — `'a'`, `'b'` — and
         * an engine that mistakes that SET of leaves for the array asks "does some element's leaf satisfy the
         * body", which is a different question with a different answer, and a strictly wider one: it returns rows
         * MongoDB excludes. `items.tags`, whose leaf genuinely IS an array, must keep matching throughout.
         */
        describe('18.37 $elemMatch below an array requires the leaf itself to be an array', () => {

            const scalarLeaf = { id: 'o', items: [{ k: 'a' }, { k: 'b' }] };
            const arrayLeaf = { id: 'o', items: [{ k: 'x', tags: ['t', 'u'] }] };
            // A dot-prop path THROUGH an array is admitted by the gate but not generated by the schema-derived
            // type, so such a filter reaches the engines only as JSON (the same gap §18.36's leaf cases name).
            const objArray = (row: ObjArray, filter: unknown) =>
                matchJavascriptObject(row, filter as WhereFilterDefinition<ObjArray>, ObjArraySchema);

            test('a scalar leaf is not an array, so $elemMatch matches nothing even when a leaf equals the operand', async () => {
                const result = await objArray(scalarLeaf, { 'items.k': { $elemMatch: { $eq: 'a' } } });
                expectOrAcknowledgeUnsupported(result, false, '$elemMatch below an array');
            });

            test('a scalar leaf is not an array, so an $elemMatch range body matches nothing either', async () => {
                const result = await objArray(scalarLeaf, { 'items.k': { $elemMatch: { $gt: 'A' } } });
                expectOrAcknowledgeUnsupported(result, false, '$elemMatch below an array');
            });

            test('a scalar leaf that no leaf equals also matches nothing (control — the answer is false either way)', async () => {
                const result = await objArray(scalarLeaf, { 'items.k': { $elemMatch: { $eq: 'zz' } } });
                expectOrAcknowledgeUnsupported(result, false, '$elemMatch below an array');
            });

            test('an ARRAY leaf below an array answers $elemMatch from its own elements', async () => {
                const result = await objArray(arrayLeaf, { 'items.tags': { $elemMatch: { $eq: 't' } } });
                expectOrAcknowledgeUnsupported(result, true, '$elemMatch below an array');
            });

            test('an ARRAY leaf below an array does not match when none of its elements satisfies the body', async () => {
                const result = await objArray(arrayLeaf, { 'items.tags': { $elemMatch: { $eq: 'zz' } } });
                expectOrAcknowledgeUnsupported(result, false, '$elemMatch below an array');
            });

            test('the leaf is judged by the value STORED there, not by the shape the schema declares', async () => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any -- deliberately non-conforming: array data under a scalar-declared leaf, which is what makes "is it an array?" a question about the VALUE
                const nonConforming = { id: 'o', items: [{ k: ['a', 'b'] }] } as any;
                const result = await objArray(nonConforming, { 'items.k': { $elemMatch: { $eq: 'a' } } });
                expectOrAcknowledgeUnsupported(result, true, '$elemMatch below an array');
            });
        });
